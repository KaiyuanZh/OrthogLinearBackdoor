...
class ResNet(nn.Module):
    ...
    def custom_forward(self, x):
        ...
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        activation['layer1'] = out.view(out.size(0), -1)
        out = self.layer2(out)
        activation['layer2'] = out.view(out.size(0), -1)
        out = self.layer3(out)
        activation['layer3'] = out.view(out.size(0), -1)
        out = self.layer4(out)
        activation['layer4'] = out.view(out.size(0), -1)
        out = F.avg_pool2d(out, 4)
        pre_out = out.view(out.size(0), -1)
        final = self.linear(pre_out)
...
class resnet18face(nn.Module):
    # feat_scale lets us deal with CelebA, other non-32x32 datasets
    def __init__(self, block=BasicBlock, num_blocks=[2,2,2,2], num_classes=8, feat_scale=1, wm=1):
        super(resnet18face, self).__init__()
    ...
    def forward(self, x, with_latent=False, fake_relu=False, no_relu=False):
        ...
        out = F.avg_pool2d(out, 4)
        # pre_out = out.view(out.size(0), -1)
        pre_out = out.reshape(out.size(0), -1)
        final = self.linear(pre_out)
        if with_latent:
            return final, pre_out
        return final
